<template>
  <div>
    ...
    <el-card>
      ...
      <el-steps align-center :active="activeIndex * 1"
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef"
        ...
        <el-tabs v-model="activeIndex" :tab-position="'left'"
          @tab-click="tabClicked" :before-leave="beforeTabLeave">
          ...
          <el-tab-pane label="商品图片" name="3">
            ...
            <el-upload :action="uploadURL" :on-preview="handlePreview"
              :on-remove="handleRemovePic" list-type="picture"
  ...
  methods: {
    async getCateList() {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('获取商品分类数据使白')
      }
    ...
    beforeTabLeave(activeName, oldActiveName) {
      // console.log('即将离开的标签页时' + oldActiveName);
      // console.log('即将进入的标签页时' + activeName);
      // return false
    ...
    async tabClicked() {
      ...
      if (this.activeIndex === '1') {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('获取列表动态参数列表失败!')
        }
